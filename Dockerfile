...
# ── Elvis Sun Swarm: Quantitative Analysis Toolkit ──────────────
# Adds Python data-science and trading packages for multi-agent quant workflows.
# Installed in a virtual environment to avoid polluting the base image.
ARG OPENCLAW_INSTALL_QUANT_TOOLS=""
RUN if [ -n "$OPENCLAW_INSTALL_QUANT_TOOLS" ]; then \
      apt-get update && \
      DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends \
        python3 python3-pip python3-venv build-essential && \
      python3 -m venv /opt/quant-venv && \
      /opt/quant-venv/bin/pip install --no-cache-dir \
        pandas numpy requests tushare ccxt scikit-learn && \
      rm -rf /var/lib/apt/lists/*; \
    fi
ENV PATH="/opt/quant-venv/bin:${PATH}"

# Install pm2 for process management in swarm deployments
RUN npm install -g pm2 --unsafe-perm 2>/dev/null || true
